...
public class HibernateQueryLanguage {
    ...
    //TODO verificar
    public static final String IS_SERVICE_AVAILABLE_BY_ID_AND_BOOKING_DATE = "select count(s) > 0 from Service s inner join s.company c where s.active is true and (s.endAt >= :bookingDate or s.endless is true) and :bookingDate between c.openAt and c.closesAt and s.id = :serviceId";
